...
Vector<DataPoint> reverseVector(Vector<DataPoint>& v) {
    int len= v.size();
...
Vector<DataPoint> topK(istream& stream, int k) {
    ...
    while(stream >> temp){
        if(count<=k){
            preTopK.enqueue(temp);
        }
        else{
            if(temp.priority>preTopK.peek().priority){
                preTopK.dequeue();
                preTopK.enqueue(temp);
            }
        }
    ...
    int len=preTopK.size();
    Vector<DataPoint> v(len);
    for (int i = 0; !preTopK.isEmpty(); i++) {
        v[i] = preTopK.dequeue();
    }
    Vector<DataPoint> topK=reverseVector(v);
    return topK;
}
