...
int sumStack(Stack<int> s) {
    return s.isEmpty()? 0: s.pop()+sumStack(s);
}
/*{
    int sum =0;
    if(s.isEmpty()){
...
STUDENT_TEST("The smallest test"){
    Queue<int> smallest ={-1};
    Queue<int> expected = {-1,-1};

    duplicateNegatives(smallest);
    EXPECT_EQUAL(smallest, expected);
